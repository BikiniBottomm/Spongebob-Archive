...
import Spongebob.modules.sql.userinfo_sql as sql
...
from Spongebob.modules.sql.afk_sql import is_afk, check_afk_status
from Spongebob.modules.sql.users_sql import get_user_num_chats
from Spongebob.modules.helper_funcs.chat_status import sudo_plus
from Spongebob.modules.helper_funcs.extraction import extract_user
from Spongebob import telethn as SaitamaTelethonClient
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        text += "\n\nThe Disaster level of this person is Poseidon."
        disaster_level_present = True
    elif user.id in DEV_USERS:
        text += "\n\nThis user is member of Krabs."
        disaster_level_present = True
    elif user.id in DRAGONS:
        text += "\n\nThe Disaster level of this person is Spongebob."
        disaster_level_present = True
    elif user.id in DEMONS:
        text += "\n\nThe Disaster level of this person is Patrick."
        disaster_level_present = True
    elif user.id in TIGERS:
        text += "\n\nThe Disaster level of this person is Pearl."
        disaster_level_present = True
    elif user.id in WOLVES:
        text += "\n\nThe Disaster level of this person is Squidward."
        disaster_level_present = True

    if disaster_level_present:
        text += ' [<a href="https://t.me/VohaUpdate/25">?</a>]'.format(
            bot.username,
        )
